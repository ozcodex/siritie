	...
	if mobkit.timer(self, 1) then
		...
		if prty < 20 then
			...
			if energy >= energy_max then
				...
				self.object:add_velocity({ x = 0, y = -0.2, z = 0 })
			end
			...
			if random() < 0.02 and not rival and light < 10 and self.hp >= self.max_hp and energy >= energy_max then
				energy = animals.place_egg(
					pos,
