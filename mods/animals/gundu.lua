...
local function brain(self)
	...
	if mobkit.timer(self, 1) then
		...
		if prty < 20 then
			...
			if
				(random() < 0.02 or energy >= 9000)
				and not rival
				and not pred
				and lightm <= 11
				and (tod < 0.5) -- only lay at night
				and self.hp >= self.max_hp
				and energy >= energy_max - 100
			then
				energy =
					animals.place_egg(pos, "animals:gundu_eggs", energy, energy_egg, "nodes_nature:salt_water_source")
			end
		end
...
animals.register_egg("animals:gundu", S("Live Gundu"), "animals_gundu_item.png", minimal.stack_max_large, energy_egg)
