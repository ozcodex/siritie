...
minetest.register_node("nodes_nature:tree_mark", {
	...
	on_timer = function(pos, elapsed)
		...
		if #positions == 0 then
			minetest.remove_node(pos)
		elseif climate.get_rain(pos, 15) or climate.time_since_rain(elapsed) > 0 then
...
for i in ipairs(tree_list) do
	...
	minetest.register_node("nodes_nature:" .. treename .. "_tree", {
		...
		groups = { tree = 1, choppy = hardness, flammable = 10 - flamesusceptibility },
	...
	minetest.register_node("nodes_nature:" .. treename .. "_log", {
		...
		paramtype2 = "facedir",
		is_ground_content = false,
		groups = { log = 1, choppy = hardness, flammable = 10 - flamesusceptibility },
		sounds = nodes_nature.node_sound_wood_defaults(),
		on_place = minetest.rotate_node,
	})
	...
	stairs.register_stair_and_slab(
		...
		crafting_station,
		"false",
		{ choppy = hardness, flammable = 8 - flamesusceptibility, woodslab = 1 },
		{
			"nodes_nature_" .. treename .. "_log_top.png",
			"nodes_nature_" .. treename .. "_log_top.png",
			"nodes_nature_" .. treename .. "_log.png",
	...
	minetest.register_node("nodes_nature:" .. treename .. "_leaves", {
		...
		walkable = false,
		climbable = true,
		groups = { choppy = 3, flammable = 2, woody_plant = 1, leafdecay = 1, leafdecay_drop = 1, compostable = 1 },
...
minetest.override_item("nodes_nature:tangkal_fruit", { stack_max = minimal.stack_max_large })
