...
function liquid_store.on_use_empty_bucket(itemstack, user, pointed_thing)
	...
	if liquiddef ~= nil and node.name == liquiddef.source then
	...
	elseif storeddef ~= nil then
		...
		local giving_back = find_stored(itemstack:get_name(), storeddef.source)
		if not giving_back then
			--nothing matches
			return nil
		end
		local new_wield = handle_stacks(user, user:get_wielded_item(), giving_back)
		minimal.switch_node(pointed_thing.under, { name = storeddef.nodename_empty })
		return new_wield
	else
...
---------------------------------------------------------
--Register liquids
liquid_store.register_liquid("nodes_nature:salt_water_source", "nodes_nature:salt_water_flowing", false)

liquid_store.register_liquid("nodes_nature:freshwater_source", "nodes_nature:freshwater_flowing", false)
